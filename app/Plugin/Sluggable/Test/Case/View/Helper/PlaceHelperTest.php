...
App::uses('View', 'View');
App::uses('PlaceHelper', 'Sluggable.View/Helper');
 ...
 */
	public function testDecodeBadResponse() {
		...
		$this->assertEquals($result, array(__d('Sluggable', 'Server lipsum.com returns error code %s.', 404)));
	}
 ...
 */
	public function testDecodeEmptyOrInvalidResponse() {
		...
		$result = $this->Place->decodeResponse($response, $noCache);
		$this->assertTrue($noCache);
		$this->assertEquals($result, array(__d('Sluggable', 'Response from lipsum.com is empty or could not be decoded.')));
 ...
 */
	public function testFetchDataConnectionFailed() {
		...
		$this->assertTrue($noCache);
		$this->assertEquals($result, array(__d('Sluggable', 'Connection to server lipsum.com failed. %s.', 'TestPlaceSocket Error')));
	}
