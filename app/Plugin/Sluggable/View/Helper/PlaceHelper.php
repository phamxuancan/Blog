 ...
 * @package Sluggable
 * @subpackage Sluggable.View.Helper
 */
 ...
 */
	protected function _decodeResponse($response, &$noCache) {
		if (!$response->isOk()) {
			$noCache = true;
			return array(__d('Sluggable', 'Server lipsum.com returns error code %s.', $response->code));
		}
		...
		$noCache = true;
		return array(__d('Sluggable', 'Response from lipsum.com is empty or could not be decoded.'));
	}
