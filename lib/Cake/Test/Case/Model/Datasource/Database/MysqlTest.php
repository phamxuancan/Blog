 ...
 */
	public function testStringConditionsParsing() {
		...
		$result = $this->Dbo->conditions("Users.id != 0 AND Users.user LIKE '%arr%'");
		$expected = " WHERE `Users`.`id` != 0 AND `Users`.`user` LIKE '%arr%'";
		$this->assertEquals($expected, $result);
 ...
 */
	public function testArrayConditionsParsing() {
		...
		$result = $this->Dbo->conditions(array('OR' => array(
			array('Users.user' => 'mariano'),
			array('Users.user' => 'nate')
		)));

		$expected = " WHERE ((`Users`.`user` = 'mariano') OR (`Users`.`user` = 'nate'))";
		$this->assertEquals($expected, $result);
 ...
 */
	public function testMixedConditionsParsing() {
		$conditions[] = 'Users.first_name = \'Firstname\'';
		$conditions[] = array('Users.last_name' => 'Lastname');
		$result = $this->Dbo->conditions($conditions);
		$expected = " WHERE `Users`.`first_name` = 'Firstname' AND `Users`.`last_name` = 'Lastname'";
		$this->assertEquals($expected, $result);
 ...
 */
	public function testReadVirtualFieldsWithNewLines() {
		...
		$this->assertRegExp('/[`\"]Users[`\"]\.[`\"]id[`\"] \+ [`\"]Users[`\"]\.[`\"]id[`\"]/', $result[7]);
	}
 ...
 */
	public function testVirtualFieldsComplexRead() {
		$this->loadFixtures('DataTest', 'Article', 'Comment', 'Users', 'Tag', 'ArticlesTag');
 ...
 */
	public function testRealQueries() {
		$this->loadFixtures('Apple', 'Article', 'Users', 'Comment', 'Tag', 'Sample', 'ArticlesTag');
 ...
 */
	public function testUpdateStatements() {
		...
		$this->Dbo->expects($this->at(1))->method('execute')
			->with("UPDATE `$db`.`articles` AS `Article` LEFT JOIN `$db`.`users` AS `Users` ON " .
 ...
 */
	public function testDeleteStatements() {
		...
		$this->Dbo->expects($this->at(1))->method('execute')
			->with("DELETE `Article` FROM `$db`.`articles` AS `Article` LEFT JOIN `$db`.`users` AS `Users` " .
