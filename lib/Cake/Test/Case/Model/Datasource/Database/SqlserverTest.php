 ...
 */
	public function testLimitOffsetHack() {
		$this->loadFixtures('Author', 'Post', 'Users');
		$query = array(
			'limit' => 2,
			'page' => 1,
			'order' => 'Users.user ASC',
		);
		$User = ClassRegistry::init('Users');
		$results = $User->find('all', $query);

		$this->assertEquals(2, count($results));
		$this->assertEquals('garrett', $results[0]['Users']['user']);
		$this->assertEquals('larry', $results[1]['Users']['user']);
		...
		$this->assertEquals('mariano', $results[0]['Users']['user']);
		$this->assertEquals('nate', $results[1]['Users']['user']);
	}
