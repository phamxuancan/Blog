 ...
 */
	public function testAssociationLazyLoading() {
		...
		$this->assertTrue(isset($Article->belongsTo['Users']));
		$this->assertFalse(property_exists($Article, 'Users'));
		$this->assertInstanceOf('Users', $Article->User);
 ...
 */
	public function testAssociationLazyLoadWithBindModel() {
		...
		$this->assertFalse(isset($Article->belongsTo['Users']));
		$this->assertFalse(property_exists($Article, 'Users'));

		$Article->bindModel(array('belongsTo' => array('Users')));
		$this->assertTrue(isset($Article->belongsTo['Users']));
		$this->assertFalse(property_exists($Article, 'Users'));
		$this->assertInstanceOf('Users', $Article->User);
	}
		...
		$options = array(
			...
			'joins' => array(
				array(
					...
					'type' => 'LEFT',
					'conditions' => array(
						'Users.id = Article.user_id',
					),
				),
			),
			'group' => array('Users.user', 'Article.published'),
			'recursive' => -1,
			'order' => array('Users.user')
		);
		$result = $TestUser->find('all', $options);
		$expected = array(
			array('Users' => array('user' => 'garrett'), 'Article' => array('published' => '')),
			array('Users' => array('user' => 'larry'), 'Article' => array('published' => 'Y')),
			array('Users' => array('user' => 'mariano'), 'Article' => array('published' => 'Y')),
			array('Users' => array('user' => 'nate'), 'Article' => array('published' => ''))
		);
 ...
 */
	public function testCrossDatabaseJoins() {
		...
		$this->loadFixtures('Article', 'Tag', 'ArticlesTag', 'Users', 'Comment');
		$TestModel = new Article();
		...
		$this->fixtureManager->loadSingle('Users', $db2);
		...
		$result = Hash::extract($TestModel->User->find('all'), '{n}.Users.id');
 ...
 */
	public function testDisplayField() {
		$this->loadFixtures('Post', 'Comment', 'Person', 'Users');
		$Post = new Post();
		$Comment = new Comment();
		$Person = new Person();
 ...
 */
	public function testPluginAssociations() {
		$this->loadFixtures('TestPluginArticle', 'Users', 'TestPluginComment');
 ...
 */
	public function testGetAssociated() {
		...
		sort($results);
		$this->assertEquals(array('Category', 'Comment', 'Tag', 'Users'), $results);
 ...
 */
	public function testAutoConstructAssociations() {
		$this->loadFixtures('Users', 'ArticleFeatured', 'Featured', 'ArticleFeaturedsTags');
		...
		$expected = array(
			'Users' => array(
				'className' => 'Users', 'foreignKey' => 'user_id',
		...
		$this->assertEquals('Users', $TestModel->User->name);
		$this->assertEquals('Users', $TestFakeModel->User->name);
 ...
 */
	public function testAutoConstructPluginAssociations() {
		...
		$this->assertFalse(isset($Comment->belongsTo['TestPlugin.Users']));
		$this->assertTrue(isset($Comment->belongsTo['Users']), 'Missing association');
 ...
 */
	public function testColumnTypeFetching() {
		...
		$model = new Article();
		$this->assertEquals('datetime', $model->getColumnType('Users.created'));
 ...
 */
	public function testCreation() {
		$this->loadFixtures('Article', 'ArticleFeaturedsTags', 'Users', 'Featured');
		$TestModel = new Test();
